...
int main(int argc, char *argv[]){
	...
	//réception du "start x" ou "wait x"
	char messageServeur[256];
	memset(messageServeur, 0, 256);
	int nbLus = recv(descripteurSocket, messageServeur, sizeof(messageServeur)-1, 0);
	if (nbLus > 0) {
		...
		if (strncmp(messageServeur, "wait", 4) == 0) {
			...
			printf("\n>>> La partie commence! Vous jouez en premier.\n");
		}
	} else {
		perror("Erreur réception start/wait ou connexion fermée");
		close(descripteurSocket);
		exit(-1);
	}
	...
	while (partie_en_cours) {
		...
		if (strcmp(messageServeur, "TURN") == 0) {
			...
			if (strcmp(messageServeur, "INFO OK") == 0) {
				...
				printf("L'adversaire a perdu une vie!\n");
			} else if (strcmp(messageServeur, "INFO ALREADY") == 0) {
				printf("L'adversaire a rejoué une lettre déjà jouée.\n");
			} else if (strcmp(messageServeur, "INFO LOSE") == 0) {
				printf("L'adversaire a perdu (plus de vies)! Vous continuez seul.\n");
